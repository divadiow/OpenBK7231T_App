...
const char* TuyaMCU_GetCommandTypeLabel(int t) {
	...
	if (t == TUYA_CMD_GET_MODULE_MAC)
		return "GetModuleMAC";
	if (t == TUYA_CMD_GET_DPCACHE)
		return "GetDPCache";
...
void TuyaMCU_OnChannelChanged(int channel, int iVal) {
	...
	// DPCache mappings are returned only when the MCU asks for the low-power/v0 DP cache.
	// Do not push them automatically from channel-change callbacks.
...
void TuyaMCU_ParseWeatherData(const byte* data, int len) {
	...
	while (ofs + 4 <= len) {
		...
		keyLen = data[ofs++];
		rawKeyLen = keyLen;
		if (ofs + rawKeyLen + 2 > len) {
			addLogAdv(LOG_INFO, LOG_FEATURE_TUYAMCU, "ParseWeatherData: truncated key at offset %i", ofs - 2);
			return;
		}
		stringData = (const char*)(data + ofs);
		if (keyLen >= (sizeof(keyBuffer) - 1))
			keyLen = sizeof(keyBuffer) - 2;
		memcpy(keyBuffer, stringData, keyLen);
		keyBuffer[keyLen] = 0;
		ofs += rawKeyLen;

		varType = data[ofs++];
		// varType: 0x00 = integer, 0x01 = string.
		valueLen = data[ofs++];
		if (ofs + valueLen > len) {
			addLogAdv(LOG_INFO, LOG_FEATURE_TUYAMCU, "ParseWeatherData: truncated value for key %s", keyBuffer);
			return;
		}
		...
		if (varType == 0x00) {
			...
			addLogAdv(LOG_INFO, LOG_FEATURE_TUYAMCU, "ParseWeatherData: key %s, valid %i, val integer %i", keyBuffer, bValid, iValue);
		}
		else if (varType == 0x01) {
			// string
			copyLen = valueLen;
			if (copyLen >= (sizeof(valueBuffer) - 1))
				copyLen = sizeof(valueBuffer) - 2;
			memcpy(valueBuffer, data + ofs, copyLen);
			valueBuffer[copyLen] = 0;
			addLogAdv(LOG_INFO, LOG_FEATURE_TUYAMCU, "ParseWeatherData: key %s, valid %i, val string %s", keyBuffer, bValid, valueBuffer);
		}
		else {
			addLogAdv(LOG_INFO, LOG_FEATURE_TUYAMCU, "ParseWeatherData: key %s, valid %i, unsupported varType %i len %i", keyBuffer, bValid, varType, valueLen);
		}
		ofs += valueLen;
	}
}
...
void TuyaMCU_ProcessIncoming(const byte* data, int len) {
	...
	case TUYA_CMD_WIFI_SELECT:
		{
			...
			if (version == 0) {
				...
				TuyaMCU_V0_ParseRealTimeWithRecordStorage(payload, payloadLen, false);
			}
			else {
				// Payload 0 = EZ/smartconfig, payload 1 = AP. OBK cannot provide Tuya EZ,
				// so keep the legacy behaviour of opening AP mode for either request.
				if (payloadLen >= 1) {
					addLogAdv(LOG_INFO, LOG_FEATURE_TUYAMCU, "ProcessIncoming: TUYA_CMD_WIFI_SELECT requested %s mode", payload[0] == 0 ? "EZ" : "AP");
				}
				g_openAP = 1;
	...
	case TUYA_CMD_QUERY_STATE:
		if (version == 0) {
			...
			TuyaMCU_V0_ParseRealTimeWithRecordStorage(payload, payloadLen, true);
		}
		else {
